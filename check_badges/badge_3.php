...
/*
    Badge B003:
    User must complete all Hard quizzes
    from Chapters 1 to 4.
*/
...
/* Check whether the user already owns B003 */
$stmt = $conn->prepare("
    SELECT ub_id
    FROM users_badges
    WHERE user_id = ?
...
$hardDone = (int)($row['hard_done'] ?? 0);

$stmt->close();

/* User has not completed all four Hard quizzes */
if ($hardDone < 4) {
    return;
}
