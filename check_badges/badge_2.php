...
/*
    Badge B002:
    User must complete all Normal quizzes
...
/* Check whether the user already owns B002 */
$stmt = $conn->prepare("
    SELECT ub_id
    FROM users_badges
    WHERE user_id = ?
 ...
 */
$stmt = $conn->prepare("
    SELECT COUNT(DISTINCT q.quiz_chap) AS normal_done
    FROM leaderboard AS l
    INNER JOIN quiz AS q
        ON q.quiz_id = l.quiz_id
    WHERE l.user_id = ?
      AND LOWER(q.quiz_type) = 'normal'
...
$normalDone = (int)($row['normal_done'] ?? 0);

$stmt->close();

/* User has not completed all four Normal quizzes */
if ($normalDone < 4) {
    return;
}
